(ns solita.etp.valvonta-kaytto.sakkopaatos-ensimmainen-postitus-test
  (:require
    [clojure.test :as t]
    [jsonista.core :as j]
...
(t/deftest sakkopaatos-ensimmainen-postitus-test
  ...
  (t/testing "Sakkopäätös / tiedoksianto (ensimmäinen postitus) toimenpide is created successfully"
    (let [valvonta-id (valvonta-service/add-valvonta! ts/*db*
                                                      {:katuosoite        "Testitie 5"
                                                       :postinumero       "90100"
                                                       :ilmoituspaikka-id 0})
          new-toimenpide {:type-id       16
                          :deadline-date (str (LocalDate/of 2023 11 4))
                          :template-id   nil
                          :description   "Sakkopäätös postitetaan liitteineen (mukana myös sakkolappu)kirjeitse kirjaamon kautta saantitodistuksella"}
